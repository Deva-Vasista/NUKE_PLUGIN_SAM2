...
class SAM2Base(torch.nn.Module):
        ...
        if multimask_output and self.samurai_mode:
            if self.kf_mean is None and self.kf_covariance is None or self.stable_frames == 0:
                ...
                if len(non_zero_indices) == 0:
                    ...
                    y_max, x_max = non_zero_indices.max(dim=0).values
                    high_res_bbox = [x_min.item(), y_min.item(), x_max.item(), y_max.item()]
                # Patch: avoid ambiguous boolean for tensor with more than one value
    ...
    def track_step(
        ...
        # Patch: handle both 7 and 9 outputs
        if isinstance(sam_outputs, (tuple, list)):
            if len(sam_outputs) == 9:
                (
                    _,
                    _,
                    _,
                ...
                raise ValueError(f"Unexpected number of outputs from sam_outputs: {len(sam_outputs)}")
        else:
            raise ValueError("sam_outputs is not a tuple or list")
